...
class AudioDiscriminator1D(nn.Module):
    """
    Defines a 1D PatchGAN discriminator for audio waveforms.
    This is based on the NLayerDiscriminator but uses Conv1d.
    """
    def __init__(self, input_nc=1, ndf=32, n_layers=2, norm_layer=nn.BatchNorm1d, use_sigmoid=False):
        """Construct a 1D PatchGAN discriminator
        Parameters:
            input_nc (int)  -- the number of channels in input images. For audio, this is typically 1.
            ndf (int)       -- the number of filters in the last conv layer
            n_layers (int)  -- the number of conv layers in the discriminator
            norm_layer      -- normalization layer
        """
        super(AudioDiscriminator1D, self).__init__()
        ...
        sequence += [nn.Conv1d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]  # output a 1-channel prediction map
        if use_sigmoid:
            sequence += [nn.Sigmoid()]
