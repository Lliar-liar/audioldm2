...
class FSQRegularizer(AbstractRegularizer):
    ...
    def __init__(
        ...
        if has_projections:
            self.project_in = nn.Linear(self.dim, effective_codebook_dim).to(self.torch_dtype)
            self.project_out = nn.Linear(effective_codebook_dim, self.dim).to(self.torch_dtype)
        else:
