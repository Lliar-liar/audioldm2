...
def normalize_lufs(waveform, sample_rate, target_lufs=-23.0):
    """
    响度归一化 (LUFS) - 这是最可能符合模型训练标准的方法
    -23.0 LUFS 是 EBU R128 广播标准
    -14.0 LUFS 是 Spotify 等流媒体平台的常见标准
    可以尝试不同的值，比如 -16.0 或 -20.0
    """
    # 确保波形是浮点数
...
if waveform_original is not None:
    ...
    # 确保两个音频长度相同
    min_length = min(len(waveform), len(waveform_original_normalized)) # <--- 使用归一化后的版本
